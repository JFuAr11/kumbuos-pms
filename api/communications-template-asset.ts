...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const templateId = sanitizePathSegment(body.templateId || "");
    const file = body.file || {};
    const name = String(file.name || "").trim();
    const mimeType = String(file.mimeType || "application/octet-stream");
    const data = String(file.data || "");

    if (!propertyId || !templateId || !name || !data) {
      res.status(400).json({ error: "propertyId, templateId, file.name, and file.data are required." });
      return;
    }

    const buffer = decodeUploadPayload(data);
    if (!buffer.length) {
      res.status(400).json({ error: "The asset payload is empty." });
      return;
    }

    if (buffer.byteLength > maxUploadBytes) {
      res.status(413).json({ error: "Template asset is too large. Upload files up to 8 MB each." });
      return;
    }

    const bucket = getFirebaseAdminStorageBucket();
    const token = randomUUID();
    const assetId = `comm-asset-${Date.now()}-${randomUUID().slice(0, 8)}`;
    const safeName = sanitizeFileName(name);
    const storagePath = `communications/${propertyId}/templates/${templateId}/${assetId}-${safeName}`;
    ...
    res.status(500).json({
      error: "Firebase Storage template asset upload failed.",
