...
type LedgerFilter = "Revenue" | "Expense" | "All";
...
export function AccountancyLedgerManager({
  ...
  const defaultType = filter === "Expense" ? "Expense" : "Revenue";
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <label className="block text-sm font-medium">
              ...
              <select
                ...
                value={editing.type}
                onChange={event => setEditing({ ...editing, type: event.target.value as AccountancyEntry["type"] })}
                disabled={filter !== "All"}
              >
            ...
            <InputField label="Category" value={editing.category} onChange={value => setEditing({ ...editing, category: value })} />
          ...
          <tbody>
            {entries.map(entry => (
              <tr key={entry.id} className="border-b border-border">
                ...
                <td className={`p-4 text-right font-semibold ${entry.type === "Revenue" ? "text-green-600" : "text-destructive"}`}>
                  {entry.type === "Expense" ? "-" : ""}{formatMoney(entry.amount, entry.currency)}
                </td>
                <td className="p-4">
                  <div className="flex justify-end gap-1">
                    <Button variant="ghost" size="icon" onClick={() => setEditing(entry)}><Edit className="h-4 w-4" /></Button>
            ...
            {!entries.length && (
              <tr>
                <td colSpan={7} className="p-8 text-center text-muted-foreground">No manual or AI ledger entries for this scope yet.</td>
              </tr>
            )}
          </tbody>
        </table>
